...
/* 
* Cuenta el numero de caracteres que tiene una cadena ingresada.
* C++ Signature: void Counting();
*/
.globl Counting
Counting:
	push {r4,r5,r6,r7,r8,r9,r10,r11,lr}
	mov r4, r0 //direccion de cadena
	mov r5, r1 //largo cadena con command
	add r4,#7 //direccion de cadena
	sub r5,#7 //largo cadena sin command
	mov r3, r5 //largo a imprimir
	ldr r0,=format 			  // formato de cadena
	mov r1,#formatEnd-format  // largo de formato
	ldr r2,=formatEnd 		  // cadena destino
	bl FormatString
	...
	mov r1,r6 //mueve el contador de caracteres
	ldr r0,=formatEnd
	bl Print
...
/* NEW
* Convierte una cadena ingresada en mayusuculas.
* C++ Signature: void CapsLock();
*/
.globl CapsLock
CapsLock:	
	push {r4,r5,r6,r7,r8,r9,r10,r11,lr}
	mov r4, r0 //direccion de cadena
	mov r5, r1 //largo cadena con command
	add r4,#6 //direccion de cadena
	sub r5,#6 //largo cadena sin command
	ldr r6, =formatoMayus
	mov r7, #0

	agregando: //concatenando cadena en mayuscula
		ldrb r8, [r4] //se carga caracter de cadena en r8
		sub r8, #32 //resta 32 para obtener la letra en mayusculas
		strb r8, [r6] //se almacena el caracter en el formato de r6
		add r4, #1 //suma direccion cadena ingresada
		add r6, #1 //suma direccion cadena a imprimir
		add r7, #1 //suma 1 al contador de caracteres
		cmp r7, r5 //se compara para determinar si ya se llego a la ultima letra
		bne agregando

	mov r1, r5 //impresion cadena en mayusculas - contador caracteres
	ldr r0, =formatoMayus //direccion cadena
	bl Print
...
formatoMayus:
	.ascii "                    "
format:
	.ascii "La cadena tiene %d caracteres"
formatEnd:
	.asciz "                                                                                                    "
